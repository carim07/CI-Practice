...
PerceptronMulticapa::PerceptronMulticapa(int cantCapas,vector<int> neuXcapa,double m,double a,double cur){
	...
	for(int i=0;i<cantidadCapas;i++){
		...
		gradiente.push_back(aux1);		//gradientes iniciales
		aux1.clear();
	}
	...
	pesos.push_back(aux3);
	difPesos.push_back(aux3);
	for(int i=1;i<cantidadCapas;i++){
		for(int j=0;j<neuronasXcapa[i];j++){
			for(int k=0;k<neuronasXcapa[i-1]+1;k++){	
	...
	for(int i=1;i<cantidadCapas;i++){
		for(int j=0;j<neuronasXcapa[i];j++){
			...
			for(int k=0;k<neuronasXcapa[i-1];k++){
				suma+= salidaXneurona[i-1][k]*pesos[i][j][k];
			}
			suma+=pesos[i][j][neuronasXcapa[i-1]];	//umbral
			salidaXneurona[i][j]=factivacion(suma);
		}
	}
...
bool PerceptronMulticapa::error(vector<double> salidaDeseada) 
{
	...
	{
		// solamente verifica si salidas y sdeseada son distintas
...
void PerceptronMulticapa::propagacionAtras(vector<double> entradas, vector<double> salidaDeseada){
	...
	*/
	...
	for(int i=0;i<neuronasXcapa[cantidadCapas-1];i++){
		gradiente[cantidadCapas-1][i]=0.5*(salidaDeseada[i]-salidaXneurona[cantidadCapas-1][i])*(1+salidaXneurona[cantidadCapas-1][i])*(1-salidaXneurona[cantidadCapas-1][i]);
	}
	...
	for(int i=cantidadCapas-2;i>0;i--){
		...
		for(int j=0;j<neuronasXcapa[i];j++){
			...
			for(int k=0;k<neuronasXcapa[i+1];k++){
				suma+=gradiente[i+1][k]*pesos[i+1][k][j];	//producto de gradiente y pesos de la capa siguiente asociados a la neurona j
			}
			gradiente[i][j]=suma*(1+salidaXneurona[i][j])*(1-salidaXneurona[i][j]);
		}
	}
	...
	for(int i=1;i<cantidadCapas;i++){	//para cada capa
		for(int j=0;j<neuronasXcapa[i];j++){	//para cada neurona
			for(int k=0;k<neuronasXcapa[i-1];k++){	//para cada peso asociados a cada neurona
				difPesos[i][j][k]=(1-alfa)*mu*gradiente[i][j]*salidaXneurona[i-1][k];
				pesos[i][j][k]+=difPesos[i][j][k];
			}
			//pesos del umbral(no tiene entradas)
			difPesos[i][j][neuronasXcapa[i-1]]=(1-alfa)*mu*gradiente[i][j];
